/**
 * Newsletter Template Generator (CommonJS version for Cloud Functions)
 * Premium Design Inspired by Wrestlenomics
 */

function generateNewsletterHTML(payload) {
  const { 
    title, 
    currentDate, 
    articles, 
...
function renderTopStory(article, baseUrl) {
...
function renderBulletStory(article, baseUrl) {
  const url = `${baseUrl}#opinion/${article.slug || article.id}`;
  return `
...
function renderStandardStory(article, baseUrl) {
  const url = `${baseUrl}#opinion/${article.slug || article.id}`;
  return `
...
function renderAdBlock(ad, placement) {
